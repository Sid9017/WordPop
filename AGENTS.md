...
- 2026-03-10 - iPhone 竖屏适配：新增 `@media (max-width: 430px)` 媒体查询，仅对 iPhone 竖屏（375~430px）生效，不影响 iPad 和桌面。调整内容：日历区域（缩小 gap/padding、放大日期字号至 15px）、学学/测测泡泡按钮（字号 11→14px、最小高度 52px）、做题页（缩小 quiz-card/option/spell-input 的 padding 和字号防溢出）、连连看（紧凑化 padding/gap）、结果页（缩小 score-ring/padding）、学习页（learn-word 32→26px、padding 缩小）、家长页（设置区/词库选择器 padding 缩小、词库网格最小列宽 120→90px）、单词浏览器（header/badge/source-tag 缩小）、邀请区域缩小。涉及文件：`src/index.css`。
- 2026-03-13 - 新词两步答题 + 日历行内抽屉。**新词题卡重构**：每个新词由原来的纯拼写（`newSpell`）改为两步式 `newWord` 题型——Step1 看英文选中文释义（en2cn 选择题），Step2 拼写单词；Step1 正确后 600ms 自动过渡到 Step2，任一步错误整题判错进入错题池；错题池循环重试直到清空。新增 `newWordStep`/`step1Pick` 状态管理两步流程，Step1 自动发音、Step2 不自动发音。**日历行内抽屉**：点击今日格子不再在格内变形显示按钮，改为在今日所在行下方展开行内抽屉面板（`grid-column: 1/-1` + `max-height` 过渡动画），下方日期行平滑下移，日历宽度不受影响，手机/iPad 均适配；抽屉内含紫色"📖 学学"和粉色"✏️ 测测"渐变按钮横排并列，已完成测试时显示提示文案。移除旧的 bubble 按钮和右侧抽屉方案。涉及文件：`src/pages/QuizPage.jsx`（`buildQuestions`/`handleAnswer`/UI 渲染/自动发音/retry 逻辑全面重构）、`src/pages/ChildPage.jsx`（日历分段渲染 + 行内抽屉插入 + ref/click-outside 更新）、`src/index.css`（移除 bubble 样式，新增 `.cal-drawer-row`/`.drawer-row-content`/`.drawer-buttons`/`.drawer-btn` 样式）。
...
- 2026-03-29 - 家长页词表加载优化：`getSelectedWordCount` 对多个预置词库合并为单次 `bank_words` HEAD + `in(bank_id)`；`getAllSelectedWords`（无 limit）改为 `in(bank_id)` 合并拉取、客户端按选中词库顺序重组；`bank_words` 分页由 `range/offset` 改为主键 `id` 键集（`id > cursor`），避免深 offset。涉及文件：`src/lib/api.js`。
- 2026-03-29 - `batchLookupWords` 对未命中本地词库的单词改为固定并发池（默认 6 路并行）请求有道，进度回调报告已完成数/剩余数。涉及文件：`src/lib/api.js`；自测脚本 `scripts/test-batch-lookup-pool.mjs`。
...
- 2026-03-29 - 家长页「已添加的单词」改为 Supabase RPC 一次合并/筛选/分页：`get_parent_word_list_stats`、`get_parent_word_summary_page`（见 `supabase-migration-parent-word-list-rpc.sql`，含 `words.uk_phonetic`）；前端 `getParentWordListStats` / `getParentWordSummaryPage`（`PARENT_LIST_PAGE_SIZE`）与 ParentPage 首屏并行双 RPC、「显示更多」键集式追加；统计与标签数字来自 stats；搜索仅词形 `strpos`（不再对未加载释义做中文筛选）。`getAllSelectedWords` / `getSelectedWordCount` 仍保留。涉及：`src/lib/api.js`、`src/pages/ParentPage.jsx`。
- 2026-03-29 - 修复 `get_parent_word_summary_page`：`bank_dedup` 透出 `ord`，供 `bank_ordered` 中 `ORDER BY bd.ord, bd.word`（原遗漏列导致 `column bd.ord does not exist`）。涉及：`supabase-migration-parent-word-list-rpc.sql`。
- 2026-03-29 - 批量查词体验：`buildWordIndex` 改为并行拉取 10 个词库 JSON；新增 `prefetchWordBankIndex` 家长页挂载时预热；`batchLookupWords` 按输入顺序逐条 `onProgress`（本地命中立即显示，有道返回一条刷新一条）；家长页批量预览展示「查询中 x/y」与行内「查询中…」/失败态。涉及：`src/lib/api.js`、`src/pages/ParentPage.jsx`、`src/index.css`。
- 2026-03-31 - 学生页日历日期串修复：`calendarDays` 与「今天」曾用 `toISOString().slice(0,10)`（UTC），在东八区等时区会把本地午夜映射成前一日，导致月底少显示一天（如 3 月最后一格显示 30）。改为本地 `localYMD` + 按 `viewYear/viewMonth` 拼格内日期。涉及：`src/pages/ChildPage.jsx`。
